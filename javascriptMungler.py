...
def etsi_ja_korvaa_muuttujat_htmlsta(teksti):
	a = re.compile(r"""
			(?<=id=")\b(?P<id>(\w|-)+?)(?=") |			
			(?<=class=")(?P<luokka>[^"]*?)(?=") |
			(?<=%-\s)\b(?P<muuttuja>\w+?)\b(?=\s%>) // ASP-tyylinen muuttuja merkkijonossa.
...
def etsi_ja_korvaa_muuttujat_csssta(teksti):
	...
	"""
	a = re.compile(r"(?P<merkkijononalkumerkki>["'])(?P<merkkijono>.*?)(?P=merkkijononalkumerkki)", re.X|re.M|re.S)
	def kas(match):
		if match.group('merkkijono') in NIMIKEKARTTA:
...
def ei_tallenneta(match):
	...
	if re_osumat['merkkijono']:
		...
		return re_osumat['merkkijononalkumerkki']+(NIMIKEKARTTA[muuttuja] if (muuttuja in NIMIKEKARTTA) else muuttuja)+re_osumat['merkkijononalkumerkki']
	if re_osumat['muuttuja']:
		muuttuja = re_osumat['muuttuja']
		return NIMIKEKARTTA[muuttuja] if (muuttuja in NIMIKEKARTTA) else muuttuja
	if re_osumat['objekti']:
...
# Mahdollistaa ohittamisen. Ei palauteta välttämättä seuraavaa vuorossa olevaa kirjainyhdistelmaa.
	...
	for t in listaus:
		...
		if t not in ohitettavat and t_pointer not in ohitettavat:
			if os.path.isdir(t_pointer) and rekursio:
				palautus.extend( etsi_kasiteltavat_tiedostot(t_pointer, ohitettavat, kevytKasiteltavat, rekursio, haettavat_tiedostotyypit))
			elif os.path.isfile(t_pointer):
				if len(t.split(".")) > 1 and t.split(".")[-1] in haettavat_tiedostotyypit:
					palautus.append(t_pointer)
	return palautus	
	
def tallennaTiedostoUuteenHakemistopuuhun(tiedostonimi, hakemisto="mungled", juurikansio=os.path.abspath('.')):
...
def komentoriviParametrit(arvolle):
	if type(arvolle) == list:
		...
		haettavat = [arvolle]
	argumentinPaikka =  sisaltaaArgumentin(haettavat)  # Use predefined mapping as a base-dictionary. Usually from a previous run of the program.
	parametrit = sys.argv[argumentinPaikka:seuraavaArgumentti(argumentinPaikka)] if argumentinPaikka else []
	return parametrit
...
def suoritus(): 
	...
	tiedostotKomentorivilta = map(lambda t: pwd +"/" + t if t.find(pwd) < 0 else t, tiedostotKomentorivilta) 
	...
	for t in tiedostotKomentorivilta:
		if os.path.isfile(t):
			tiedostot.push(t)
		if os.path.isdir(t):
			tiedostot.extend(etsi_kasiteltavat_tiedostot(t, ohitettavatTiedostot, kevytKasiteltavat, rekursiivisesti))
